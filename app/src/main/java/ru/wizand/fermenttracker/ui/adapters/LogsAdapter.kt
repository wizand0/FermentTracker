...
class LogsAdapter : ListAdapter<BatchLog, LogsAdapter.VH>(DIFF) {
    ...
    inner class VH(private val b: ItemLogBinding) : RecyclerView.ViewHolder(b.root) {
        fun bind(log: BatchLog) {
            b.tvTimestamp.text = SimpleDateFormat("yyyy-MM-dd HH:mm", Locale.getDefault()).format(Date(log.timestamp))
            b.tvWeight.text = log.weightGr?.toString() ?: "N/A"
            log.photoPath?.let { path ->
                val bitmap = BitmapFactory.decodeFile(path)
                b.ivPhoto.setImageBitmap(bitmap)
            } ?: run { b.ivPhoto.setImageDrawable(null) }
        }
    }

    companion object {
        private val DIFF = object : DiffUtil.ItemCallback<BatchLog>() {
            override fun areItemsTheSame(oldItem: BatchLog, newItem: BatchLog) = oldItem.id == newItem.id
            override fun areContentsTheSame(oldItem: BatchLog, newItem: BatchLog) = oldItem == newItem
        }
    }
}